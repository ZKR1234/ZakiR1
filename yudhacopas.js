...
const starts = async (dha = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('ZAKIR', {
		font: 'Standard',
	...
	console.log(color('[ IYA BANG GW SIH OWH AJA\nTAPI BANYAK YG PEDULI ]\n\n', 'orange'), color('\n======TQ TO======\n•MHANKBARBAR\n•ZAKIR\n•IKYADS\n•KurrXd\n•DAN CREATOR BOT LAINNYA', 'yellow'))
	console.log(color('\n\nTETAP BERUSAHA JGN MUDAH MENYERAH\nKALAU ADA YG MENJATUHKAN\nPUKUL KEPALANYA', 'pink'))
	console.log(color('\n\n[ BISMILLAH SKIN LEGEND, BUKAN EPEP YA AJG', 'red'))
	dha.browserDescription = ["Mx.ZKR BOT BOT", "Chrome", "3.0.0"];

	// Menunggu QR
	dha.on('qr', () => {
		console.log(color('[', 'pink'), color('!', 'red'), color(']', 'pink'), color('NIH BRO SCAN'))
	})

	// Menghubungkan
	fs.existsSync(`./${setting.sessionName}.json`) && dha.loadAuthInfo(`./${setting.sessionName}.json`)
	dha.on('connecting', () => {
		console.log(color('[ TUNGGU BENTAR ]', 'yellow'), color('SAMPE KIAMAT...'));
	})
const spinner = { 
  "interval": 120,
  "frames": [
	...
	//connect
	dha.on('open', () => {
		console.log(color('[ Mx.ZKR BOT ]', 'yellow'), color('UDAH CONNECT BRO'));
	})
