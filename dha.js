...
petik = '.'
fake = '© ZakiR HoriZontaL'//GANTI NAMA KAMU BEP
ban =[]
...
module.exports = dha = async (dha, mek) => {
	try {
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = dha.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "© ZakiR  HoriZontaL","listType": "SINGLE_SELECT","sections": list}}, {})
      ...
      const promoteAdmin = async function(to, target=[]){
           ...
           reply(`Hm..... @${mentioned[0].split('@')[0]} ADMIN BARU BULLY`)
}
}
}
      const demoteAdmin = async function(to, target=[]){
           ...
           reply(`Hm..... @${mentioned[0].split('@')[0]} WKWK JADI MEMBER`)
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/sherlynn.jpg`)},"title": `© ZakiR HoriZontaL`,"description": "Mx.ZKR BOT", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "Mx.ZKR BOT","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}
...
*Apa bang?? bewan
*Nih nomor Owner wa.me/6281227836734`
               buttons = [{buttonId: `${prefix}command`,buttonText:{displayText: 'INSTAGRAM'},type:1},{buttonId: `${prefix}rules`,buttonText:{displayText: 'RULES BOT'},type:1},{buttonText:{displayText:'SIMPLE MENU'},type:1}]
               ...
               buttonsMessage = {
               contentText: `${menu}`,
               footerText: '© ZakiR  HoriZontaL', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
*Cek list Server GTIVD dibawah!*`
              ...
              dha.relayWAMessage(prep)
                break
                case 'infoig':
             reply(`Follow Instagram Owner : https://instagram.com/m.xzkr_`)
             break
        case 'command':
               list = []
       ...
       case 'othercmd':
               ...
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: '© ZakiR  HoriZontaL ' + nombor++,
                    rows: [
                       {
                        title: `${listmenuu[startnum++]}`,
                        description: ``,
       ...
       case 'store':
               ...
               listmsg(from, `${ucapanWaktu}`,   `© ZakiR  HoriZontaL`, list)
               break
//------------------< Game >------------------- 
        case 'limitgame': 
        case 'balance': 
...
case 'buttonstik':
              gopeynya = 'https://ibb.co/tYYSTWg/IMG-20211018-WA0009.jpg'
...
case 'tiktok':
              ...
              imageMsg = (await dha.prepareMessageMedia(fs.readFileSync(`./media/sherlynn.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/sherlynn.jpg`)})).imageMessage
              buttonsMessage = {footerText:'Mx.\n ZKR BOT', imageMessage: imageMsg,
       ...
       case 'img2url':
         ...
         case 'asupan': // shansekai                
              ...
              .then(async (body) => {
               ...
               sendMediaURL(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, '\`\`\`ASUPAN BRO GUA DAPAT DARI Mx.ZKR BOT\`\`\`')
...
//------------------< Stalk >-------------------
      case 'stalkgithub':
      case 'githubstalk':
              if (args.length == 0) return reply(`Example: ${prefix + command} ZKR1234`)
              reply(mess.wait)
              username = args[0]
...
`
      ...
      case 'stalkig':
      case 'igstalk':
             if (args.length == 0) return reply(`Example: ${prefix + command} m.xzkr_`)
             reply(mess.wait)
             username = args[0]
...
//------------------<18+ Menu>-----------------------   
       ...
       if (!isOwner) return reply(mess.only.owner)
              ...
              dha.sendMessage(from, '*PERMINTAAN:* '+bokep+'\n*DOSA TANGGUNG PRIBADI*\n*NI BRO FREE BUAT KAMU DOWNLOAD SENDIRI:* '+ kep, text, { quoted: ftoko, caption: `FIKI BNYK STOK`})
              break
                case 'xnxx':
...
case 'ada':
 if (!mek.key.fromMe && !isOwner) return
	dha.toggleDisappearingMessages(from, 0)
	dha.toggleDisappearingMessages(from, 0)
	dha.toggleDisappearingMessages(from, 0)
 break
...
dha.sendMessage(from, tapib1, document, { quoted: mek, filename:`Mx.ZKR BOT ~ 404 ${vipi}.mp3`, mimetype: 'audio/application' })
...
"caption": "https://chat.whatsapp.com/GY74IwuwlFELw97ByRk79",
"contextInfo": {
"forwardingScore": 3,
"isForwarded": true
}
...
"sellerJid": "6281227836734@s.whatsapp.net",
"token": "AR7i5IXXiMA6NjT0DxcwdcKxhXCy1rOrvlNJzqXPMr8PCg==",
"totalAmount1000": "Rp 999.000.00",
...
dha.sendMessage(_.jid, 
			{"contentText": `*「 Siaran Bot Boardcast 」*\n© Eroze Botz\n*Isi Pesan :* ${body.slice(4)}`,
			"footerText": '© ZakiR  HoriZontaL',
			"buttons": [
      ...
      case 'demote':
       ...
       case 'kickall': // Anti Banned
             if (!isOwner) return reply(mess.only.owner)
              ...
              break
       case 'leave':
           if (!mek.key.fromMe && !isOwner) return reply('Only  Owner!!')
              setTimeout( () => {
              dha.groupLeave(from) 
              }, 2000)
              setTimeout( () => {
...
reply('_' + err + '_\n\n' + js)
       ...
       if (budy.includes(`@6281227836734`)) {
